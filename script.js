...
const five = strings.filter(function (ans) {
  return ans.length >= 5;
});
console.log(five);
...
const square = numbers.map(function (ans) {
  return ans ** 2;
});
console.log(square);
...
const add = alphabets.reduce(function (ans, output) {
  return ans + output;
});
console.log(add);
...
const find = number.find(function (ans) {
  return ans % 2 == 0;
});
console.log(find);
...
const rank = ranks.findIndex(function (ans) {
  return ans === 7;
});
console.log(rank);
...
//3:8
const num = [2, 4, 6, 8];
const item = num.forEach(function (item, index) {
...
const forty = marks.some(function (forty) {
  return forty < 45;
});
console.log(forty);
...
const yes = positive.every(function (yes) {
  return yes > 0;
});
console.log(yes);
...
console.log(person);
// 2
console.log(person.hasOwnProperty('vehicles')) &&
